...
const verticalScale = size => (height / guidelineBaseHeight) * size;   //margin top and bottom for only vertical
const moderateScale = (size, factor = 0.5) => size + (scale(size) - size) * factor; //for height and width
const moderateScaleVertical = (size, factor = 0.5) => size + (verticalScale(size) - size) * factor; //for top and bottom 
const textScale = percent => {  //for font size
